...
import { getRendezVousByAssociation, getRendezVousByDonateur, getRendezVousByEntreprise } from '../../features/rendezVousSlice';
...
const RendezVousPage = () => {
    ...
    const { objets } = useSelector((state) => state.storeObjets);
    const initFetch = useCallback(() => {
      if(user && !user.donateur &&  (user.user.role==="donateur")  ) {
      dispatch(getRendezVousByDonateur(Number(id))).then(er=>console.log(er));
      console.log("les  :",desRendezVous)
  }
      else
       if(user && !user.donateur && user.user.role==="association"){
          dispatch(getRendezVousByAssociation(Number(id))).then(er=>console.log(er));
     console.log("les rendezVous de  :",desRendezVous)}
     else 
     if(user &&  user.donateur && user.donateur.user.role==="entreprise"){
     dispatch(getRendezVousByEntreprise(Number(id))).then(er=>console.log(er));}
      }, [dispatch]) 
